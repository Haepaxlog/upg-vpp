...
var _ = ginkgo.Describe("UPG Binary API", func() {
	...
	ginkgo.Context("for PFCP Session Server", func() {
		...
		ginkgo.It("Configures PFCP Heartbeats", func() {
			...
			hbConfigStr, er := f.VPP.Ctl("show upf heartbeat-config")
			...
			_, er = f.VPP.Ctl("upf pfcp heartbeat-config timeout 5 retries 15")
			gomega.Expect(er).To(gomega.BeNil())
			...
			gomega.Expect(err).To(gomega.BeNil())
			gomega.Expect(hbGetReply.Timeout).To(gomega.BeEquivalentTo(5))
			gomega.Expect(hbGetReply.Retries).To(gomega.BeEquivalentTo(15))
